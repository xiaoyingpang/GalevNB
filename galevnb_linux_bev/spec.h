...
void star2mag(star2mag_option* s2mopt,
              ...
              double fluxes[array_size])
{
    //double* fluxes = new double[array_size];
    ...
    for (int i=0; i<number_of_filters; i++) {
        fluxes[i] = spec->integrate_flux(s2mopt->ugrizH.at(i));
...
void internaleComputeMags(double* flux1,double* flux2,double mags[array_size])
{
    for (int i=0; i<number_of_filters; i++) {
        mags[i] = -2.5 * log10(flux1[i] + flux2[i]) - zeropoint_mags[ zeropoint_ids[ i ] ][ i ];
    }
}
...
extern "C" {
	...
	void startomag_(double* p_lteff, double* p_mstar,double* p_lbol,double* p_Zmet,double* fluxes)
        {
             star2mag( opt, p_lteff, p_mstar, p_Zmet, p_lbol, fluxes);
	}

    void computemag_(double* flux1,double* flux2,double* mag_array)
    {
        internaleComputeMags(flux1,flux2,mag_array);
    }
